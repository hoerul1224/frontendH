...
export default function ManageConsultation() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      ...
      await API.post(`/consultation/admin/${selectedUser._id}`, form);
    }
    setSaved(true);
    setEditingId(null);
  ...
  const handleCancelEdit = () => {
    ...
    <>
      ...
      {(showForm || editingId) && (!isReadOnly || editingId) && (
            ...
            <tbody>
              {allRecords.map((r) => (
                <tr key={r._id}>
                  ...
                  <td>{r.doctorName ? `dr. ${r.doctorName}` : '-'}</td>
                  <td>{r.complaint || '-'}</td>
                  <td>{r.diagnosis || '-'}</td>
                  ...
                  {!isReadOnly && (
                    <td>
                      <button type="button" className="btn-edit-table" onClick={() => handleEditClick(r)}>
                        Edit
                      </button>
                    </td>
                  )}
